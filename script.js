...
let nivel = document.querySelector(".nivel"); // Variable para gestionar el nivel actual del juego (quizás el contenedor principal)
let puntajeElemento = document.querySelector(".score"); // Variable para llevar la puntuación del jugador (elemento HTML donde se muestra)
let bandera = document.querySelector(".bandera"); // Referencia a la bandera del nivel (elemento HTML)
let botonCambiarColor = document.querySelector(".cambiar-color"); // Botón para cambiar el color de fondo (elemento HTML)
let botonFinalizarJuego = document.querySelector(".finalizar"); // Botón para terminar el juego (elemento HTML)
...
let marioDerecha = 1; // Elemento HTML de Mario
let marioVelocidad = 15;
...
const sonidoBandera = new Audio("./sonidos/Level_Clear.wav");
const sonidoHongo = new Audio("./sonidos/Powerup.wav");
...
function agarrarMoneda() {
  if (moneda.style.display != "none") {
    ...
    if (
      ...
      sonidoMoneda.play();
      puntajeElemento.innerHTML = parseInt(puntajeElemento.innerHTML) + 1;
      TieneMoneda = true;
    }
  }
}
...
function detectarColisionBandera() {
  ...
  if (
    ...
    if (marioGrande && TieneMoneda) {
      ...
      sonidoBandera.play(); // Reproduce sonido de victoria
...
botonFinalizarJuego.onclick = function () {
  setTimeout(() => {
    ...
    nivel.innerHTML = "Game Over Clara"
    Gameover.play();
    TemaFondo.pause();
    juegoActivo = false; 
